...
export default function HomeV2StudentShowcase() {
  ...
  const trackX = useTransform(scrollYProgress, [0.1, 0.9], ['0%', '-12%']);

  const scrollBy = (dir: 'left' | 'right') => {
    if (!scrollerRef.current) return;
    const card = scrollerRef.current.querySelector<HTMLElement>('[data-card]');
    const delta = card ? card.offsetWidth + 24 : 380;
  ...
  return (
    <section ref={sectionRef} className="relative bg-white py-24 md:py-32 overflow-hidden">
      <motion.div
        style={{ x: trackX }}
        aria-hidden
        className="pointer-events-none absolute top-12 left-0 right-0 select-none"
      >
        <div className="whitespace-nowrap font-display text-[140px] md:text-[220px] font-bold text-[#F5FBFF] tracking-[-0.04em] leading-none">
      ...
      <div className="relative mx-auto max-w-[1280px] px-5 md:px-10">
        <div className="grid lg:grid-cols-12 gap-6 items-end mb-12">
          <Reveal direction="up" className="lg:col-span-8">
            ...
            <h2 className="mt-4 font-display text-[40px] md:text-[60px] lg:text-[72px] font-bold text-[#111827] tracking-[-0.04em] leading-[0.98]">
          ...
          <div className="lg:col-span-4 flex lg:justify-end gap-3">
            <button
              ...
              className="grid h-12 w-12 place-items-center rounded-full border border-[#e5e7eb] bg-white text-[#252525] hover:border-[#2563eb] hover:text-[#2563eb] transition-colors"
            >
            ...
            <button
              ...
              className="grid h-12 w-12 place-items-center rounded-full bg-[#2563eb] text-white shadow-[0_12px_28px_-10px_rgba(37,99,235,0.55)] hover:bg-[#1d4ed8] transition-colors"
            >
      ...
      <div className="relative">
        <div aria-hidden className="pointer-events-none absolute inset-y-0 left-0 w-16 z-10 bg-gradient-to-r from-white to-transparent" />
        <div aria-hidden className="pointer-events-none absolute inset-y-0 right-0 w-16 z-10 bg-gradient-to-l from-white to-transparent" />

        <div
          ref={scrollerRef}
          className="relative flex gap-6 overflow-x-auto snap-x snap-mandatory scroll-px-5 md:scroll-px-10 px-5 md:px-10 pb-6 pt-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
          {stories.map((s, i) => (
            <motion.article
              key={s.name}
              ...
              className="group relative snap-start shrink-0 w-[300px] sm:w-[360px] md:w-[420px] rounded-3xl border border-[#e5e7eb] bg-white overflow-hidden hover:shadow-[0_30px_60px_-30px_rgba(37,99,235,0.3)] hover:border-[#dbeafe] transition-all duration-500"
            >
              <div className={`relative h-[260px] bg-gradient-to-br ${s.accent} overflow-hidden`}>
